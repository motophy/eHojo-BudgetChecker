...
```bash
pip install xlrd==1.2.0 xlsxwriter==3.2.9 customtkinter==5.2.2
```
...
**Q. 실행 시 "No module named" 오류가 납니다.**
A. `pip install xlrd==1.2.0 xlsxwriter==3.2.9 customtkinter==5.2.2` 를 다시 실행하세요.

**Q. 결과 파일에 집행 내역이 없습니다.**
A. 예산서의 부서코드·사업코드와 집행내역 파일의 코드가 일치하는지 확인하세요.
   두 파일 모두 같은 연도·기간 기준으로 내려받아야 합니다.

**Q. 결과 파일을 열면 수식 오류(#REF!)가 표시됩니다.**
A. 파일을 열 때 Excel에서 "콘텐츠 사용" 또는 "편집 허용"을 클릭하면 수식이 정상 계산됩니다.
